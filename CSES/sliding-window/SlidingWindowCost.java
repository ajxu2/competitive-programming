// created: 01-07-2024 Sun 09:31 PM
...
public class SlidingWindowCost {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // sort
        Arrays.sort(b);
        BIT bit = new BIT(n), bit2 = new BIT(n);
        ...
        for (int i = k-1; i < n; i++) {
            // extract answer
            int med = bit.lower_bound((k+1)/2);
            long cnt1 = bit.qry(0, med), cnt2 = bit.qry(med+1, n-1);
            long sum1 = bit2.qry(0, med), sum2 = bit2.qry(med+1, n-1);
            io.print(b[med] * cnt1 - sum1 + sum2 - b[med] * cnt2 + " ");
            // prepare next window
            int tmp = Arrays.binarySearch(b, a[i-k+1]);
            bit.add(tmp, -1);
            bit2.add(tmp, -b[tmp]);
            if (i != n-1) {
                tmp = Arrays.binarySearch(b, a[i+1]);
                bit.add(tmp, 1);
                bit2.add(tmp, b[tmp]);
            }
        }
